...
export function TextScramble({ text, className = "", autoPlay = false, autoPlayDelay = 0 }: TextScrambleProps) {
  ...
  useEffect(() => {
    if (autoPlay) {
      const timeout = setTimeout(() => scramble(), autoPlayDelay)
  ...
  return (
    ...
    >
      <span className="relative font-sans tracking-widest uppercase">
        {displayText.split("").map((char, i) => (
          <span
            key={i}
            className={`inline-block transition-all duration-150 ${
              isScrambling && char !== text[i] ? "text-[#c83a30]" : ""
            }`}
      ...
      <span className="relative mt-1 h-px w-full overflow-hidden">
        <span
          className={`absolute inset-0 bg-current transition-transform duration-500 ease-out origin-left ${
